...
export function createAuthRouter(runtime: PezhwanRuntime): Router {
  ...
  const byUser = (req: Request) => (req as PezhwanRequest).pezhwan?.userId ?? req.ip ?? '';

  router.post(
    '/register',
    rateLimit(runtime, { type: 'register' }),
    async (req: PezhwanRequest, res: Response) => {
      const { email, phone, password, metadata } = (req.body ?? {}) as Record<string, unknown>;
  ...
  router.post(
    ...
    async (req: PezhwanRequest, res: Response) => {
      ...
      if (typeof refreshToken !== 'string' || !refreshToken) {
        throw new AuthenticationError('Refresh token is required', 'REFRESH_TOKEN_REQUIRED');
      }
  ...
  router.post(
    ...
    async (req: PezhwanRequest, res: Response) => {
      ...
      if (!channel || !target || !code) {
        throw new ValidationError('channel, target and code are required', 'OTP_PARAMS_REQUIRED');
      }
...
export function createSessionRouter(runtime: PezhwanRuntime): Router {
  ...
  router.post(
    ...
    async (req: PezhwanRequest, res: Response) => {
      ...
      await runtime.sessions.revokeAll(req.pezhwan.userId, runtime.config.applicationId);
