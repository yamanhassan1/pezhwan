...
| TOTP secret leak at rest                | Info Disclosure `[Documented]` | `mfaSecret` is stored envelope-encrypted at rest under a per-tenant master key (AES-256-GCM, `v3:` DEK envelope) — `mfa.service.ts`; hidden with `select:false` — `models/user.model.ts:64`. Legacy `v2:`/unprefixed envelopes remain decryptable during the migration window (`security-audit.md` §5.4, `docs/operations/mfa-migration.md`). |
...
| Tenant-injection via API key      | Elevation of Privilege                   | API keys are globally-unique unguessable hashes; lookup not tenant-filtered but finds exactly one row whose tenant is _assigned_, never caller-supplied — `apiKey.service.ts:51-58`.                                                                                                                                                                                                                         |
