...
| --- | --- |
| Node.js | **>= 20** (check the `engines` field in the root `package.json`). |
| npm | Comes with Node 20+. The repo uses npm workspaces. |
...
application, and `ADMIN` role, plus the admin user when configured):
