...
Two credential styles, both sent as `Authorization: Bearer <token>`:

| Style | Obtained from | Used by |
| --- | --- | --- |
| Access token | `POST /v1/auth/login` / `register` / `refresh` (RS256 JWT, ~15 min TTL) | Interactive callers; refresh the token via `/v1/auth/refresh`. |
| API key | Admin API `POST /v1/admin/api-keys` (single-use `rawKey`, `pk_…` value) | Service-to-service (`/v1/services/ping`); only the hash is stored server-side. |
