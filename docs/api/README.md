...
(`react`, `node`, `express`, `python`, `go`, `java`, `dotnet`, `cli`) is a thin
...
| MFA        | `/v1/mfa/setup \| enable \| verify \| disable \| login` | Bearer required (login is public)       |
...
| Admin      | `/v1/admin/status`, `/v1/admin/healthz`, `/v1/admin/metrics/flush` | `ADMIN` role for write actions          |
...
```json
{ "success": false, "error": { "code": "AUTHENTICATION_FAILED", "message": "...", "requestId": "..." } }
```
...
- **API key** — server-to-server, presented as `Authorization: Bearer pk_live_...`.
