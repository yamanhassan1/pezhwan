...
app.post(
  ...
  h(async (req) => {
    ...
    if (!email || !password)
      throw { code: 'MISSING_FIELDS', message: 'email and password required', status: 400 };
...
app.post(
  ...
  h(async (req) => {
    ...
    if (result.mfaRequired || !result.tokens) {
      throw {
        code: 'MFA_REQUIRED',
        message: 'MFA enabled — demo only supports password',
        status: 400,
      };
    }
    return {
...
app.post(
  ...
  h(async (req) => {
    ...
    if (!code || !state)
      throw { code: 'MISSING_FIELDS', message: 'code and state are required', status: 400 };
    const entry = pending.get(state);
    if (!entry)
      throw { code: 'UNKNOWN_STATE', message: 'Unknown or expired state parameter', status: 400 };
